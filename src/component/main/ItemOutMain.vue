...
import { showSku, skuOutReasons } from "@/port/sku";
import { itemOut } from "@/port/item";
...
const itemOutService = () => {
  ...
  itemOut(props.id, reason.value, count.value, "")
      .then(() => {
 ...
 */
onMounted(() => {
  skuOutReasons().then((res) => {
    reasons.value = res;
    reason.value = reasons.value[1];
  });
